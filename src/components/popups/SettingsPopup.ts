...
export const createSettingsPopup = ({
  ...
  const renderTopBar = () => {
    ...
    const barY = dimensions.navigationY + dimensions.navigationHeight * 0.7;
    const totalWidth = dimensions.popupWidth - 40;
    const buttonWidth = Math.min(120, totalWidth / Math.max(buttons.length, 1) - 10);
    const buttonHeight = 40;
    
    buttons.forEach((button, index) => {
      const buttonX = barX + (index * (buttonWidth + 10));
      const isActive = activeSection === button.id;
  ...
  const renderContent = () => {
    ...
    if (activeContentSection) {
      ...
      sectionContentContainer.y = dimensions.contentY + (activeContentSection.title ? 40 : 0);
